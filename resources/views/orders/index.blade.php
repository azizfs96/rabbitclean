                                    ...
                                    @foreach ($orders as $order)
                                        <tr class="{{ $order->is_show ? '' : 'bg-color' }}">
                                            ...
                                            <td class="py-1">
                                                ...
                                                <br><small class="text-muted">{{ $order->pick_hour ? $order->getTime(substr($order->pick_hour, 0, 2)) : 'N/A' }}</small>
                                            </td>
                                            <td class="py-1">
                                                {{ \Carbon\Carbon::parse($order->delivery_date)->format('M d, Y') }}
                                                <br><small class="text-muted">{{ $order->delivery_hour ? $order->getTime(substr($order->delivery_hour, 0, 2)) : 'N/A' }}</small>
                                            </td>
                                            <td class="py-1">
                                                {{ currencyPosition($order->total_amount) }}
                                                @if($order->admin_adjusted_amount && $order->admin_adjusted_amount != $order->total_amount)
